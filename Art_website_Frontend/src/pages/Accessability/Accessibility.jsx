        ...
        <>
            <div className="m-20 mt-5 mb-10">
                ...
                <div>
                    ...
                    <p>If you have difficulty accessing any part of this website, please reach out to our support team at <a href="mail:accessability@musetaleart.com" className="underline hover:text-orange_web font-semibold text-eerieBlack" target="_blank">accessibility@musetalesart.com</a> or call us at <b className="font-semibold">+92 349 5445560</b>. We will make every effort to assist you promptly.</p>
                    <form action={action}>
                        <label htmlFor="name">Name</label>
                        <input type="text" id="name" name="name" defaultValue={data?.name} placeholder="Enter your name" className="border border-dim_gray w-full p-2 mt-1 mb-1 focus:outline" />
                        {data?.nameError ? <span style={{ color: "red" }}><br />{data?.nameError}</span> : null}
                        <br />
                        <label htmlFor="email">Email</label>
                        <input type="email" id="email" name="email" defaultValue={data?.email} placeholder="Enter your email" className="border border-dim_gray w-full p-2  mt-1 mb-1 focus:outline" />
                        {data?.emailError ? <span style={{ color: "red" }}><br />{data?.emailError}</span> : null}
                        <br />
                        <label htmlFor="message">Message</label>
                        <textarea rows={3} id="message" name="message" defaultValue={data?.message} placeholder="Write a message..." className="w-full p-2 border border-dim_gray resize-none overflow-y-auto focus:outline" />
                        {data?.messageError ? <span style={{ color: "red" }}><br />{data?.messageError}</span> : null}
                        <div className="text-center">
                            <button className="border border-dim_gray hover:bg-orange_web hover:text-white_web mt-5 pl-10 pr-10 rounded" disabled={pending}>{pending ? "Submitting..." : "Submit"}</button>
                        </div>
